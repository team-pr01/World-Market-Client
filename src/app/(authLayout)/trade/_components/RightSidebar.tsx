...
const RightSidebar = ({onBuyClick, onSellClick, createHoverOverlay, removeHoverOverlay, investmentAmount, setInvestmentAmount, tradeLines}) => {
    console.log(tradeLines, "tradelines");
  const { placeTrade, activeTrade } = useSocket();
      ...
      <div className="p-3">
        ...
        <div className="rounded border border-gray-800 mt-4">
          ...
          <div className="p-3 max-h-[250px] overflow-y-auto space-y-2">
            {tradeLines?.trades?.length > 0 ? (
              tradeLines?.trades?.map((trade, index) => (
                <TradeListItem
                  key={index}
                  trade={{
                    id: trade._id,
